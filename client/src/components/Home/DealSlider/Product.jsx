...
const Product = ({ image, name, offer, tag }) => {
    return (
        <Link to="/products" className="flex   flex-col items-center gap-1.5 p-6 cursor-pointer ">
            <div className="w-52 h-44  transform hover:scale-110 transition-transform duration-150 ease-out bg">
                <img draggable="false" className="w-full h-full object-cover" src={image} alt={name} />
            </div>
